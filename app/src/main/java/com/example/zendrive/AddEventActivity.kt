...
class AddEventActivity : AppCompatActivity() {
    ...
    // Tracks dynamically added metadata field views
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Use the singleton DB directly — ViewModel doesn't expose insertEvent with returned ID,
        // which is needed to associate EventMeta rows with the correct event.
        ...
        btnSaveEvent.setOnClickListener {
            ...
            // Collect non-empty metadata pairs from dynamic views
            ...
            // lifecycleScope runs on Main dispatcher — safe to call Toast / finish() directly
            lifecycleScope.launch {
                val eventId = db.vehicleEventDao().insertEvent(newEvent).toInt()

                if (extraMetaList.isNotEmpty()) {
                    db.eventMetaDao().insertAllMeta(
                        extraMetaList.map { EventMeta(eventId = eventId, key = it.first, value = it.second) }
                    )
                }

                // Keep vehicle odometer in sync if the new reading is higher
                if (odometer != null) {
